    ...
    for entry in std::fs::read_dir(src)? {
        ...
        if ft.is_dir() {
            let rel = from
                ...
                .to_string_lossy()
                .replace('\\', "/");
            if exclude.iter().any(|e| rel == e.trim_end_matches('/')) {
                continue;
            }
            copy_dir_recursive_excluding(&from, &to, root, exclude)?;
        } else if ft.is_file() {
            std::fs::copy(&from, &to)?;
        }
...
impl TestServer {
    ...
    pub async fn with_fixture_and_options(name: &str, initialization_options: Value) -> Self {
        ...
        let exclude: Vec<String> = initialization_options["excludePaths"]
            .as_array()
            .map(|a| {
                a.iter()
    ...
    pub async fn assert_stays_responsive(
        &mut self,
        slow_method: &str,
    ...
    /// Poll `$/php-lsp/debugStats` until at least `n` `warm_start_indexes`
    /// replays have completed. Returns `false` on timeout (~15 s).
    pub async fn wait_for_warm_start_replays(&mut self, n: u64) -> bool {
        for _ in 0..300 {
            let resp = self.client.request_no_params("$/php-lsp/debugStats").await;
            if resp["result"]["warm_start_replays_completed"]
                .as_u64()
                .unwrap_or(0)
                >= n
            {
    ...
    pub async fn change_configuration(&mut self, value: Value) -> Value {
        ...
        self.client
            .wait_for_log_message_starting_with("php-lsp: using PHP ")
            .await
    }
